...
//! payloads to a sidecar.
//!
//! Why: DVD-Video MPEG-2 carries NTSC Line-21 closed captions inside
//! the `user_data` extension. The target output strips those bytes from
//! the video elementary stream (so the result is "pure" MPEG-2 video) and
...
//! written yet).
//!
//! Without this filter a typical NTSC title keeps thousands of user_data
//! blocks (megabytes of caption data) inside the video elementary stream.
//!
//! ## Algorithm
//!
...
//!    them back too.
//!
//! Discs can carry `zero_byte` stuffing between a GOP header and the
//! `00 00 01 B2` user_data start code; that stuffing is stripped along
//! with the user_data block.
...
/// currently load-bearing for byte-identical output; other
...
mod tests {
    ...
    /// Real-disc pattern: GOP header followed by 124 `zero_byte`
    /// stuffing bytes, then a user_data block carrying EIA-608 CC data,
    /// then `00 00 01 00` picture start. Both the stuffing and the
    /// user_data are stripped.
    ...
    /// preserved (e.g. 4 zero bytes before the first slice header).
    #[test]
    ...
    /// Real-disc regression: a GOP whose body bytes end in `0x00` (because the
    ...
    /// the following block. Observed discs have had 0 such zeros, but
    /// the rule should be symmetric.
    #[test]
    ...
    /// Reproduces a regression observed on a real disc:
    ...
    fn picture_start_after_gop_passes_through() {
        // Real-disc byte pattern: GOP header then picture
