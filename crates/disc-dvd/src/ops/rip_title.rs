//! `rip-title` operation — produce per-track outputs for
//! one DVD title.
//!
//! Files produced:
//!
...
//! routing. Video output has `user_data` blocks stripped to a sidecar
//! so the elementary stream is pure MPEG-2 video.
...
pub fn run(reader: &crate::DvdReader, params: Params) -> Result<Report> {
    ...
    for (stream_n, mut h) in std::mem::take(&mut audio_handlers) {
        ...
        // Rename to include the delay literal in the filename.
...
fn open_subpicture_handler(
    ...
    // regardless of the stream's IFO position (per-track extraction).
