//! `demux-title` operation — walk a title's PGC cells directly from disc
//! and demultiplex each cell's MPEG-PS sectors into per-stream files,
//! wiring cell metadata (especially `stc_discontinuity`) through to the
//! demuxer so step-5b's FAP-based audio resync fires at the right
//! boundaries.
//!
...
pub fn run(reader: &DvdReader, params: Params) -> Result<Report> {
    ...
    if cells.is_empty() {
        return Err(anyhow!("PGC for title {} has no cells", params.title));
    }
