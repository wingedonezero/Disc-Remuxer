...
/// checked on a grayscale CLUT (Y=191->0xCB, 54->0x2C, 28->0x0D);
/// rounding would yield 0xCC / 0x0E. The chroma terms are the standard
/// full-range coefficients but UNVERIFIED — confirm on a colored-subtitle
/// disc via byte-compare.
#[must_use]
...
mod tests {
    ...
    fn ycrcb_matches_reference_clut() {
        // A grayscale CLUT (Cr=Cb=0x80). These four distinct luma levels
        // map to the expected RGB — full-range expansion, floor (not
        // round; round would give 0xCC and 0x0E).
