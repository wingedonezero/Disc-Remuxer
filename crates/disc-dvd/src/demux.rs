...
impl Demuxer {
    ...
    ///
    /// The demuxer does **naive concatenation** of PES payloads across
    /// cell boundaries (including `stc_discontinuity` boundaries) — no
    /// FAP-resync, no trailing-frame truncation, no PTM-aware byte drops.
    ///
    /// Every byte of the elementary stream is kept as emitted by the
    /// original encoder; the PTS-side discontinuity
    ...
    pub fn process_pes(&mut self, pes: &PesPacket<'_>) -> Result<(), DemuxError> {
        ...
        // No FAP-resync, no cell-boundary truncation: naive concatenation
        // of PES payloads (after stripping the PES header + BD common
        // header) produces the expected AC-3 elementary streams on a
...
mod tests {
    ...
    /// emitted verbatim. No FAP-based drop.
    #[test]
