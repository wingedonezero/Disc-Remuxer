...
pub struct DemuxSummary {
    ...
    /// observed. Kept for the per-cell sanity check in CLI commands.
...
impl Demuxer {
    ...
    /// [`Self::process_sector`] (or [`Self::process_pes`]).
    ///
    /// Verified 2026-05-22 against MakeMKV's mkvextract output: all
    /// four AC-3 streams on ANGEL_S1D1 title 1 (23 cells, 9 with
    /// stc_discontinuity, ~44 minutes) byte-match MakeMKV's tracks
    /// SHA-for-SHA when our demuxer does **naive concatenation** of
    /// PES payloads across cell boundaries — no FAP-resync, no
    /// trailing-frame truncation, no PTM-aware byte drops.
    ///
    /// MakeMKV preserves every byte of the elementary stream as
    /// emitted by the original encoder; the PTS-side discontinuity
    /// handling lives in the container layer (PTS resets in MKV
    /// block metadata), not in the codec bytes themselves.
    ///
    /// So this method exists for callers that want to track per-cell
    /// progress / log boundaries (and for future use cases like the
    /// VobSub PTS index which DOES need cell-boundary awareness),
    /// but it does **not** modify the audio output bytes.
    pub fn begin_cell(&mut self, stc_discontinuity: bool) {
        if stc_discontinuity {
            self.discontinuity_boundaries += 1;
            log::debug!(
                "demux: stc_discontinuity boundary observed (#{})",
                self.discontinuity_boundaries
            );
        }
    }
    ...
    pub fn process_pes(&mut self, pes: &PesPacket<'_>) -> Result<(), DemuxError> {
        ...
        // No FAP-resync, no cell-boundary truncation. Verified against
        // MakeMKV's mkvextract output 2026-05-22: naive concatenation
        // of PES payloads (after stripping the PES header + BD common
        // header) produces byte-identical AC-3 elementary streams on a
        // multi-cell title with 9 stc_discontinuity boundaries.
...
mod tests {
    ...
    #[test]
    fn begin_cell_counts_discontinuities() {
        let tmp = std::env::temp_dir().join("disc-remuxer-demux-test-count");
        ...
        demux.begin_cell(true);
        demux.begin_cell(false);
        assert_eq!(demux.discontinuity_boundaries, 2);
        std::fs::remove_dir_all(&tmp).ok();
    }

    /// Verify the post-revert behaviour: across an `stc_discontinuity`
    /// boundary, EVERY byte that follows the BD substream header is
    /// emitted verbatim. No FAP-based drop. This is the byte-level
    /// behaviour that matches MakeMKV's `mkvextract` output on
    /// ANGEL_S1D1 title 1 (4 AC-3 streams, all SHA-identical).
    #[test]
    fn begin_cell_does_not_drop_audio_bytes() {
        let tmp = std::env::temp_dir().join("disc-remuxer-demux-test-no-drop");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
        let mut demux = Demuxer::new(&tmp);
        ...
        // Cell boundary with discontinuity — must NOT drop subsequent bytes.
        demux.begin_cell(true);
        assert_eq!(demux.discontinuity_boundaries, 1);

        // Second PES — even with a non-zero FAP, the bytes that USED
        // to be skipped by 5b's FAP-resync must now be emitted intact.
        ...
        let _ = demux.finish().unwrap();
        let body =
            std::fs::read(tmp.join(StreamKey::Ac3(0).filename())).unwrap();
        // pes1: 4 bytes after 3-byte BD-common strip.
        // pes2: ALL 8 payload bytes after the 3-byte BD-common strip
        // (including the leading 0xDE 0xAD 0xBE 0xEF that 5b wrongly
        // dropped).
        assert_eq!(&body[..4], &[0x0B, 0x77, 0xAA, 0xBB]);
        assert_eq!(&body[4..12], &[0xDE, 0xAD, 0xBE, 0xEF, 0x0B, 0x77, 0xCA, 0xFE]);
        std::fs::remove_dir_all(&tmp).ok();
    }
}
